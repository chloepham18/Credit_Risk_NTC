...
fico10_labels = [f"{i}-{i+10}" for i in fico10_bins[:-1]]
...
dti10_bins = list(np.arange(0, 1.1, 0.1)) + [np.inf]  # 0.0, 0.1, ..., 1.0, inf
dti10_labels = [f"{int(left*100)}–{int(right*100)}%" for left, right in zip(dti10_bins[:-2], dti10_bins[1:-1])] + ['>100%']
df_cleaned['DTI_Band_10pct'] = create_bins(df_cleaned['DTI'], dti10_bins, dti10_labels)

# Loan Band
loan_bins = [0, 1000, 5000, 10000, 25000, np.inf]
loan_labels = [
    'No debt (0–1K)',
    'Low (1K–5K)',
    'Moderate (5K–10K)',
...
# Asset Band
asset_bins = [0, 1000, 2000, 3000, np.inf]
asset_labels = ['<1K', '1K–2K', '2K–3K', '>3K']
